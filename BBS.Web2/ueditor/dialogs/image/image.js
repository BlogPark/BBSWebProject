...
(function () {
    var g = $G,
        ...
        maskIframe = g("maskIframe"); //tab遮罩�?用来解决flash和其他dom元素的z-index层级不一致问�?
       // flashObj;                   //flash上传对象
    ...
    function searchImage() {
        ...
        var reqCallBack = function (data) {
            ...
            for (var i = 0, len = imgObjs.length; i < len - 1; i++) {
                ...
                img.src = obj.objURL; //obj.thumbURL 为缩略图，只能针对百度内部使�?
    ...
    /**
     * 依据url中的地址创建一个预览图片并将对应的信息填入信息框和预览�?
     */
                        ...
                        ajax.request(editor.options.imageManagerUrl, {
                            ...
                            onsuccess:function (xhr) {
                                ...
                                for (var k = 0, ci; ci = imageUrls[k++];) {
									...
									var scrCurr = $getswitchdomian();//此方法在internal.js文件�?使用时请正确配置域名
									//alert("scrCurr--->0000:"+scrCurr);
									img.setAttribute(k < 35 ? "src" : "lazy_src", scrCurr + ci.replace(/\s+|\s+/ig, ""));
                                    img.setAttribute("_src", scrCurr + ci.replace(/\s+|\s+/ig, ""));
    ...
    /**
     * TAB切换
     * @param tabParentId  tab的父节点ID或者对象本�?
     */
